...
struct Mount_Point_Ops {
    ...
    int (*SetAcl)(struct Mount_Point *mountPoint, const char *path, int uid, int permissions);
    int (*SetSetUid)(struct Mount_Point *mountPoint, const char *path, int setUid);
};
...
struct File_Ops {
    ...
    int (*Clone)(struct File *file, struct File **pClone); /* Create a new File for underlying data source. */
};
...
int Seek(struct File *file, ulong_t offset);
