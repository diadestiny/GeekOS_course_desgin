    ...
    struct Kernel_Thread* kthread, struct User_Context* userContext)
{
    ...
    Debug ("Setup_User_Thread() pid=%d, thread=0x%08lx, stack=0x%08lx, uc=0x%08lx\n",
...
static void Reaper(ulong_t arg)
{
	...
	if ((kthread = s_graveyardQueue.head) == 0) {
		...
		Debug ("Reaper: pid=%d disposing of thread @ 0x%08x, stack @ 0x%08x\n",
...
int PrintProcessList(void)
{
    ...
    char *n[6] = {"", "(mainThread)", "(Idle)", "(Reaper)", "(Floppy Requests)", "(IDE Requests)"};
    int uid;
    ...
    while(kthread)
    {
        ...
        if (uc) {
            ...
            Debug("PPL: mem=0x%08lx arg=0x%08lx pargv=0x%08lx argv=0x%08lx sz=%d\n",
                (ulong_t)uc->memory, uc->argBlockAddr, pargv, argv, sizeof(int) + sizeof(char**));
            int i = 0;
            *buf = '\0';
            for (i=0; __copy_from_user (uc, buf+i, argv+i, 1) && i < MAX_CMD_LEN; i++)
                if (buf[i] == '\0')
                    break;
            buf[MAX_CMD_LEN-1] = '\0';
            cmd = buf;
            ...
            cmd = (kthread->pid >= 1 && kthread->pid <= 5) ? n[kthread->pid] : "(kernel)";

        Print("%3d %3d %5ld %8d %8d %10d %7d %s\n",
        kthread->pid, uid, kthread->numTicks, kthread->priority, kthread->refCount,
...
#endif
    struct ThreadsSemaphore *kthread_sem = Get_Front_Of_Thread_Semaphore_List(&(kthread->SemaphoreList));
    while (kthread_sem!=0)
    {
#ifdef SEMAPHORE_DEBUG
        //Print("Found semaphore %s (%d)\n",kthread_sem->semaphore->sem_name, kthread_sem->semaphore->sem_id);
        Print("Found semaphore (%d)\n", kthread_sem->semaphore->sem_id);
#endif
        DestroySemaphore(kthread_sem->semaphore->sem_id, kthread);
        // Attention: Get_Next cannot be used here, because DestroySemaphore frees kthread_sem
...
void Wake_Up_One(struct Thread_Queue* waitQueue)
{
	...
	Debug ("Wake_Up_One: waking up %08X from %08X\n",
               (unsigned int)best, (unsigned int)g_currentThread);
    }
}
