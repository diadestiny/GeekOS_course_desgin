...
struct Page *Find_Page_To_Page_Out()
{
    ...
    switch (g_currentPagingAlgorithm) {
       case PAGING_WSCLOCK:	/* 类 LRU algorithm */
          if (!last)  last = g_pageList;
          curr = last + 1;
          i = 0;
          while (i++ < s_numPages) {//寻找未使用时间超过WORKING_SET_TIMEOUT阈值的页面
             if (curr >= g_pageList + s_numPages)  curr = g_pageList;
          ...
          if (!best) {		/* 找不到,则随机淘汰一个页面*/
              ...
              while ( !(best->flags & PAGE_PAGEABLE) ) {
                  best++;
                  if (best >= g_pageList + s_numPages)  best = g_pageList;
              }
