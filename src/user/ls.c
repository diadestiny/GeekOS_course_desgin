...
//  struct VFS_ACL_Entry owner = stat->acls[0];
    struct VFS_ACL_Entry user;
    bool isDir = stat->isDirectory;
    bool isSetuid = stat->isSetuid;
    bool noneSet = true;
    int i;
    ...
    for (i=0; i < VFS_MAX_ACL_ENTRIES; i++) {
        if (stat->acls[i].permission) {
            noneSet = false;
            break;
        }
    }

    Print("%c%c ", isDir ? 'd' : '-', isSetuid ? 's' : '-');
    for (i=0; i < VFS_MAX_ACL_ENTRIES; i++) {
        user = stat->acls[i];
        if (!i || user.permission || noneSet) {
             Print ("%c%c%c:%d ",
                 user.permission & O_READ  ? 'r' : '-',
                 user.permission & O_WRITE ? 'w' : '-',
                 user.permission & O_EXCL  ? 'e' : '-',
                 user.uid);
        }
        if (noneSet)  break;
    }
